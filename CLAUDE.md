...
- [lib/auth/reautenticacion.ts](lib/auth/reautenticacion.ts) — **un solo intento de re-autenticación por pestaña**, y no es un detalle cosmético: un 401 tanto puede ser una sesión vencida —volver a entrar la arregla— como una autorización que va a fallar siempre (la cuenta sin acceso a Fabric, un ID token que no pasa la verificación del servidor). Cerrar sesión ante el segundo caso manda al usuario a Microsoft, que con SSO lo devuelve autenticado en el acto, el 401 se repite y se vuelve a cerrar la sesión: un ciclo infinito de redireccionamiento. Por eso el `logout()` automático pasa por `intentarReautenticar()`; a la segunda ya no redirige, avisa y deja al usuario en la app con el botón "Cerrar sesión" del sidebar. Una carga sin 401 limpia la marca (sessionStorage, para sobrevivir al redirect). **Toda respuesta a un 401 va por ahí, nunca con `logout()` directo.**
...
- [lib/graphql.ts](lib/graphql.ts) — cliente genérico contra la GraphQL API de **Microsoft Fabric SQL Database**. Lee `NEXT_PUBLIC_FABRIC_GRAPHQL_ENDPOINT` de env; sin token o sin endpoint lanza `GraphQLRequestError` de inmediato, y marca `sesionExpirada` cuando Fabric responde 401. Usa `cache: 'no-store'`. Dos funciones: `fetchGraphQL()` lanza ante cualquier error, y `ejecutarGraphQL()` devuelve la respuesta cruda para tolerar **errores parciales** (lo que necesita la consulta agrupada). Ver "Límite de tasa" abajo. El **401 y el 403 no son lo mismo y no se colapsan**: el 401 es la sesión (`sesionExpirada`) y el 403 es que la cuenta no tiene acceso al elemento dentro de Fabric (`sinAcceso`) —los app roles de Entra no conceden permisos de Fabric, esos se dan en el workspace—, así que volver a iniciar sesión no lo arregla. En los dos casos se lee el cuerpo de la respuesta: ahí viene la razón real, y sin ella todo fallo se veía igual que "modo offline". Y hay un tercer caso que **no es ninguno de los dos**: Fabric responde **200 con errores** y `data: null` cuando el usuario sí tiene permiso sobre el elemento de la API pero la conexión al origen rechaza su identidad (`"The request to data source failed with authentication error"`). Con la conexión en SSO, Fabric pasa la identidad a la base SQL, así que el permiso falta **dentro de la base** —no en Entra ni en los app roles—; `explicarFallo()` traduce ese texto a algo accionable en vez de enseñar el inglés crudo. Fabric manda un error por campo raíz, así que el mismo problema llega siete veces: `getDatosOperativos()` los deduplica.
